...
public class ExpressionBuilder {
    ...
    Expression processConditionalOrExpression(BenefitParser.ConditionalOrExpressionContext conditionalOrExpressionContext) {
        if (null != conditionalOrExpressionContext.conditionalAndExpression() && !conditionalOrExpressionContext.conditionalAndExpression().isEmpty()) {
            Expression lhsExpression = processConditionalAndExpression(conditionalOrExpressionContext.conditionalAndExpression());
            if (null != conditionalOrExpressionContext.connectorOr() && !conditionalOrExpressionContext.connectorOr().isEmpty()) {
                ...
                lhsExpression = processConditionalOrExpression(conditionalOrExpressionContext.conditionalOrExpression());
                booleanExpressions.add(lhsExpression);
                //Expression booleanExpression = processConditionalAndExpression(conditionalOrExpressionContext.conditionalAndExpression());
                booleanExpressions.add(rhsExpression);
            ...
            //Expression lhsExpression = processConditionalAndExpression(conditionalOrExpressionContext.conditionalAndExpression());
            Expression lhsExpression = processConditionalOrExpression(conditionalOrExpressionContext.conditionalOrExpression());
    ...
    Expression processConditionalAndExpression(BenefitParser.ConditionalAndExpressionContext conditionalAndExpressionContext) {
        if (null != conditionalAndExpressionContext.relationalExpression() && !conditionalAndExpressionContext.relationalExpression().isEmpty()) {
            Expression lhsExpression = processRelationalExpression(conditionalAndExpressionContext.relationalExpression());
            if (null != conditionalAndExpressionContext.connectorAnd() && !conditionalAndExpressionContext.connectorAnd().isEmpty()) {
                List<Expression> booleanExpressions = new ArrayList<>();
                Expression rhsExpression = lhsExpression;
                lhsExpression = processConditionalAndExpression(conditionalAndExpressionContext.conditionalAndExpression());
                booleanExpressions.add(lhsExpression);
                //Expression booleanExpression = processRelationalExpression(conditionalAndExpressionContext.relationalExpression());
                booleanExpressions.add(rhsExpression);
            ...
            //Expression lhsExpression = processRelationalExpression(conditionalAndExpressionContext.relationalExpression());
            Expression lhsExpression = processConditionalAndExpression(conditionalAndExpressionContext.conditionalAndExpression());
    ...
    Expression processRelationalExpression(BenefitParser.RelationalExpressionContext relationalExpressionContext) {
        if (null != relationalExpressionContext.additiveExpression() && !relationalExpressionContext.additiveExpression().isEmpty()) {
            ...
            if (null != relationalExpressionContext.relationalExpression() && !relationalExpressionContext.relationalExpression().isEmpty()) {
                ...
                lhsExpression = processIterativeStatement(relationalExpressionContext);
                return lhsExpression;
            }else{
                return lhsExpression;
            }
        ...
        private ArithmeticOperator resolveOperatorForRelationalContext (TerminalNode
        operator, BenefitParser.RelationalExpressionContext relationalExpressionContext,
...
*/
        ...
        Expression processAdditiveExpression (BenefitParser.AdditiveExpressionContext additiveExpressionContext){
            if (null != additiveExpressionContext.multiplicativeExpression()) {
                Expression lhs = processMultiplicationExpression(additiveExpressionContext.multiplicativeExpression());
                if (null != additiveExpressionContext.ADD() && null != additiveExpressionContext.additiveExpression()) {
                    Expression rhs = lhs;
                    lhs = processAdditiveExpression(additiveExpressionContext.additiveExpression());
        ...
        multiplicativeExpressionContext){
            if (null != multiplicativeExpressionContext.unaryExpression()) {
                Expression lhs = processUnaryExpression(multiplicativeExpressionContext.unaryExpression());
                if (null != multiplicativeExpressionContext.MUL() && null != multiplicativeExpressionContext.multiplicativeExpression()) {
                    Expression rhs = lhs;
                    lhs = processMultiplicationExpression(multiplicativeExpressionContext.multiplicativeExpression());
