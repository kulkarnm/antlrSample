...
public class DummyEventProcessor {
    ...
    public Scheme processDummyEvent(Scheme scheme, DummyEvent dummyEvent){
        try {

            List<Field> eventFields = Arrays.asList(dummyEvent.getClass().getDeclaredFields());
            for (Field eventField : eventFields) {
                ...
                str = str.toUpperCase();
                Object value = eventField.get(dummyEvent);
